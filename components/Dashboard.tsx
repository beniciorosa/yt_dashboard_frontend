...
import { Plus, Users, Video, ChevronRight, Loader2, Trash2, GripVertical, LayoutGrid, List, RefreshCw, AlertTriangle, ArrowUpDown, Pin, Clapperboard, TrendingUp, User } from 'lucide-react';

interface Props {
  onSelect: (id: string) => void;
}

type SortOption = 'custom' | 'subscribers' | 'videos' | 'growth' | 'newVideos';
  ...
  const renderCard = (comp: any, index: number, isDraggable: boolean) => {
    ...
    if (viewMode === 'grid') {
        return (
            <div 
                ...
                className={`rounded-lg shadow-sm transition-all duration-200 cursor-pointer group relative overflow-hidden h-full flex flex-col p-4 ${
                    comp.isMyChannel 
    ...
    return (
        <div 
            ...
            onDragOver={handleDragOver}
            onDrop={(e) => isDraggable && handleDrop(e, comp.id)}
            className={`rounded-lg shadow-sm transition-all duration-200 cursor-pointer group relative overflow-hidden flex flex-row items-center py-2 px-4 gap-4 ${
                comp.isMyChannel 
